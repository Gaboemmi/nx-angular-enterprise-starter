...
3. Keep it framework-independent TypeScript. When an Nx boundary is justified,
   use the context's `scope:<bounded-context>` with `type:domain`. It must not
   import Angular, HTTP, DTOs, persistence models, state libraries, or provider
   SDKs.
