...
3. Place it in the owning context's application responsibility. If that
   responsibility has an Nx boundary, use `scope:<bounded-context>,type:application`.
   Depend on domain models and explicit ports, never on Angular UI, HTTP clients,
   DTOs, or provider SDKs.
